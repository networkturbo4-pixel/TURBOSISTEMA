...
1.  **Método de Ingreso por PIN:** Los usuarios inician sesión exclusivamente mediante su `PIN` de 8 dígitos único por usuario (administradores, técnicos y clientes).
2.  **RBAC (Roles y Permisos):** Cada usuario tiene asignado un Rol (ej. "admin", "tecnico", "cliente"). El acceso a módulos específicos se valida leyendo el rol de la sesión.
