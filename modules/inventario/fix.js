const fs = require('fs');
let c = fs.readFileSync('c:/xampp/htdocs/TURBOSAAS/modules/inventario/inventario.js', 'utf8');

c = c.replace(/wrap\.style\.display = 'block'; empty\.style\.display = 'none';\s*grid\.innerHTML = res\.data\.map\(\(p, i\) => `/, "if (!window.selectedProducts) window.selectedProducts = new Set();\n                window.lastProductsData = res.data;\n                wrap.style.display = 'block'; empty.style.display = 'none';\n                grid.innerHTML = res.data.map((p, i) => `");

c = c.replace(/<tr class="product-row" data-product-id="\$\{p\.id\}"/, '<tr class="product-row ${window.selectedProducts && window.selectedProducts.has(p.id) ? "row-selected" : ""}" data-product-id="${p.id}"');

c = c.replace(/<td data-label="Producto">/, `<td style="text-align:center;width:40px;vertical-align:middle;">
...
c = c + addStr;
fs.writeFileSync('c:/xampp/htdocs/TURBOSAAS/modules/inventario/inventario.js', c);
