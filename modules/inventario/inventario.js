...
(function() {
    ...
    function initTabs() {
        ...
        document.querySelectorAll('.inv-toolbar-tabs .inv-tab[data-tab]').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                const tbRight = document.querySelector('.inv-toolbar-right');
                if (tbRight) tbRight.style.display = btn.dataset.tab === 'productos' ? 'flex' : 'none';
        ...
        try {
            ...
            if (res.success && res.data.length > 0) {
                ...
                grid.innerHTML = res.data.map((p, i) => {
                    const isChecked = window.selectedProducts && window.selectedProducts.has(p.id) ? 'checked' : '';
                    const type = p.product_type || 'normal';
                    return `<tr class="product-row ${isChecked ? 'row-selected' : ''}" data-product-id="${p.id}" data-product-type="${type}" data-name="${(p.name||'').toLowerCase()} ${(p.description||'').toLowerCase()} ${(p.searchable_children||'').toLowerCase()}" data-cat="${p.category_id||''}" data-disp="${p.qty_disponible||0}" data-crit="${p.stock_critico||0}" data-malo="${p.qty_malogrado||0}" data-total="${p.total_quantity||0}" data-inst="${p.qty_instalado||0}" style="animation: fadeIn 0.25s ease forwards; animation-delay: ${i*0.03}s; opacity: 0;">
                        ...
                        <td data-label="Producto">
                            <div style="display:flex; align-items:center; gap:10px;">
                                ...
                                ${p.display_image 
                                    ? `<img src="${BASE}/${p.display_image}" class="lb-thumb" data-lb-src="${BASE}/${p.display_image}" data-lb-caption="${esc(p.name)}" style="width:38px;height:38px;border-radius:10px;object-fit:cover;flex-shrink:0;border:1px solid var(--border-color);cursor:zoom-in;">` 
                                    : (p.requires_photos == 1 
    ...
    window.applyProductTypeVisibility = function(type) {
        ...
        if (type === 'normal') {
            ...
            if (skuScannerCol) skuScannerCol.style.display = 'grid';
    ...
    window.rebuildVariantTableHeaders = function(type = selectedProductType) {
        const thead = document.getElementById('variantsTableHead');
        if (!thead) return;
        const hasCost = document.getElementById('varHasCustomCost')?.checked;
        let html = '<tr>';
        if (type === 'bundle') {
            html += '<th style="width:60px;text-align:center;"><i class="ph ph-image"></i> Foto</th>';
        }
        html += '<th style="min-width:160px;">Nombre</th>';
        variantColumns.forEach(c => { html += `<th style="min-width:100px;">${esc(c.name)}</th>`; });
        if (hasCost) html += '<th style="min-width:100px;">Costo (S/)</th>';
        if (type === 'bundle') {
            html += '<th style="min-width:100px;">U. Medida</th>';
        }
    ...
    function rebuildVariantTableRows() {
        ...
        tbody.querySelectorAll('.variant-input-row').forEach(row => {
            ...
            // Try to preserve photo input if possible by cloning
    ...
    function addVariantRowWithData(name, quantity, costo, attrs, unit_type, existingFileInput) {
        ...
        tdName.innerHTML = `<input type="text" class="form-control" data-field="name" value="${esc(name || '')}" placeholder="Nombre variante" style="font-size:0.85rem;">`;
        ...
        if (hasCost) {
            ...
            tdCost.innerHTML = `<input type="number" class="form-control" data-field="costo" step="0.01" min="0" value="${costo || ''}" placeholder="0.00" style="font-size:0.85rem;">`;
        ...
        if (type === 'bundle') {
            ...
            tdUnit.innerHTML = `<select class="form-select" data-field="unit_type" style="font-size:0.85rem;">
                ...
                <option value="Kilos" ${unit_type === 'Kilos' ? 'selected' : ''}>Kilos</option>
                <option value="Litros" ${unit_type === 'Litros' ? 'selected' : ''}>Litros</option>
                <option value="Metros" ${unit_type === 'Metros' ? 'selected' : ''}>Metros</option>
        ...
        tdQty.innerHTML = `<input type="number" class="form-control" data-field="quantity" min="1" value="${quantity || 1}" style="font-size:0.85rem;">`;
        tr.appendChild(tdQty);

        // 7. Delete
        const tdDel = document.createElement('td');
        tdDel.innerHTML = `<button type="button" class="btn-delete-row" onclick="removeVariantRow(this)" title="Eliminar"><i class="ph ph-x"></i></button>`;
        ...
        try {
            ...
            if (res.success && res.data.length > 0) {
                ...
                res.data.forEach((child, idx) => {
                    ...
                    const imageHtml = child.product_image ? `<img src="${BASE}/${child.product_image}" style="width:32px;height:32px;border-radius:4px;object-fit:cover;border:1px solid var(--border-color);">` : '';
                    tr.innerHTML = `
                        <td data-label="Variante">
                            <div style="display:flex; align-items:center; gap:8px; padding-left:24px;">
                                ...
                                ${imageHtml}
                                <div>
                                    <div style="font-weight:600;color:var(--text-color);font-size:0.9rem;">${esc(child.name)} <span style="font-size:0.75rem; color:var(--text-muted); font-weight:normal;">(${esc(child.unit_type || 'Unidades')})</span></div>
    ...
    window.quickAddReprintInput = async function() {
        ...
        try {
            ...
            if (res.success && res.data && res.data.length > 0) {
                // If exact match or single result, add directly
    ...
    async function generateLabels() {
        ...
        skusToRender.forEach(s => {
            ...
            for (let c = 0; c < labelCopies; c++) {
                ...
                const dateHtml = (showDate && s.created_at) ? `<div class="label-date-text">${s.created_at.split(' ')[0]}</div>` : '';

                if (labelType === 'barcode') {
                    html += `<div class="label-item">${logoHtml}${companyHtml}${nameHtml}<svg id="${id}"></svg>${skuHtml}${dateHtml}</div>`;
                } else {
                    html += `<div class="label-item">${logoHtml}${companyHtml}${nameHtml}<div id="${id}" style="margin:0 auto;"></div>${skuHtml}${dateHtml}</div>`;
                }
            }
        });
        html += '</div>';
