...
(function() {
    ...
    function initTabs() {
        ...
        document.querySelectorAll('.inv-toolbar-tabs .inv-tab[data-tab]').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                if (btn.dataset.tab === 'escaner') {
                    setTimeout(() => {
                        const scannerInput = document.getElementById('scannerInput');
                        if (scannerInput) scannerInput.focus();
                    }, 100);
                    if (typeof renderScannerHistory === 'function') renderScannerHistory();
                }
            });
        });
    }
    ...
    async function loadMetrics() {
        try {
            ...
            if (res.success && res.data) {
                const elTot = document.getElementById('metricTotal');
                if (elTot) elTot.textContent = res.data.total ?? 0;
                const elDisp = document.getElementById('metricDisponible');
                if (elDisp) elDisp.textContent = res.data.disponible ?? 0;
                const elInst = document.getElementById('metricInstalado');
                if (elInst) elInst.textContent = res.data.instalado ?? 0;
                const elLow = document.getElementById('metricLowStock');
                if (elLow) elLow.textContent = res.data.low_stock ?? 0;
                const elProd = document.getElementById('metricProductos');
                if (elProd) elProd.textContent = res.data.productos_registrados ?? 0;
            }
    ...
    // ══════════════════════════════════════════════
    // ── Smart Scanner Hub Logic ───────────────────
    // ══════════════════════════════════════════════
    let scannerScanHistory = [];
    try {
        const saved = sessionStorage.getItem('turbosistema_scanner_history');
        if (saved) scannerScanHistory = JSON.parse(saved);
    } catch(e) {}

    function playScannerBeep() {
        const soundEnabled = document.getElementById('scannerSound')?.checked ?? true;
    ...
    function saveScannerHistory() {
        try {
            sessionStorage.setItem('turbosistema_scanner_history', JSON.stringify(scannerScanHistory.slice(0, 50)));
        } catch(e) {}
    }

    function addScannerHistory(data) {
        if (!data) return;
        const skuCode = data.sku_code || 'SIN-CODIGO';
        // Remove previous duplicate to bring to top
        const existingIdx = scannerScanHistory.findIndex(h => h.sku_code === skuCode);
        if (existingIdx !== -1) {
            scannerScanHistory.splice(existingIdx, 1);
        }
        scannerScanHistory.unshift({
            timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' }),
            id: data.id,
            sku_code: skuCode,
            product_id: data.product_id,
            product_name: data.product_name || 'Producto',
            product_image: data.sku_thumbnail || data.product_image || '',
    ...
    function renderScannerHistory() {
        const body = document.getElementById('scannerHistoryBody');
        const badge = document.getElementById('scannerHistoryBadge');
        if (!body) return;
        if (badge) badge.textContent = `${scannerScanHistory.length} escaneo${scannerScanHistory.length === 1 ? '' : 's'}`;

        if (scannerScanHistory.length === 0) {
            body.innerHTML = `<tr><td colspan="8" style="text-align:center;padding:24px;color:var(--text-muted);"><i class="ph ph-barcode" style="font-size:1.8rem;display:block;margin-bottom:6px;opacity:0.3;"></i>Aún no se han escaneado productos en esta sesión.</td></tr>`;
            return;
        }

        body.innerHTML = scannerScanHistory.map((item, idx) => {
            const status = item.status || 'disponible';
            const thumb = item.product_image 
                ? `<img src="${BASE}/${item.product_image}" data-lb-src="${BASE}/${item.product_image}" data-lb-caption="${esc(item.product_name)}" style="width:36px;height:36px;border-radius:8px;object-fit:cover;cursor:pointer;border:1px solid var(--border-color);" alt="">` 
                : `<div style="width:36px;height:36px;border-radius:8px;background:var(--bg-color);border:1px dashed var(--border-color);display:flex;align-items:center;justify-content:center;"><i class="ph ph-image" style="color:var(--text-muted);font-size:1rem;opacity:0.5;"></i></div>`;
            
            const custody = item.assigned_user_name 
                ? `<span class="badge" style="background:rgba(59,130,246,0.12);color:#3b82f6;display:inline-flex;align-items:center;gap:4px;padding:4px 8px;border-radius:6px;font-size:0.75rem;"><i class="ph ph-user-circle"></i> ${esc(item.assigned_user_name)}</span>`
                : `<span style="color:var(--text-muted);font-size:0.75rem;"><i class="ph ph-minus"></i> Sin asignar</span>`;

            return `<tr>
                <td style="color:var(--text-muted);font-size:0.8rem;font-family:monospace;">${esc(item.timestamp)}</td>
                <td style="text-align:center;">${thumb}</td>
                <td><code style="font-weight:700;font-size:0.88rem;color:var(--primary-color);">${esc(item.sku_code)}</code></td>
                <td><span style="font-weight:600;font-size:0.88rem;">${esc(item.product_name)}</span></td>
                <td><span style="font-size:0.8rem;color:var(--text-muted);">${esc(item.category_name)}</span></td>
                <td><span class="status-badge status-${esc(status)}">${esc(status).toUpperCase()}</span></td>
                <td>${custody}</td>
                <td style="text-align:center;">
                    <button type="button" class="btn btn-sm btn-primary" onclick="reopenScannedItem(${idx})" style="padding:4px 10px;font-size:0.78rem;display:inline-flex;align-items:center;gap:4px;">
    ...
    window.reopenScannedItem = function(idx) {
        const item = scannerScanHistory[idx];
        if (!item || !item.raw_data) return;
        renderScannerResultCard(item.raw_data);
        openScannedItemInterface(item.raw_data);
    };

    function renderScannerResultCard(d) {
        ...
        const type = d.product_type || (d.is_bulk == 1 ? 'granel' : 'normal');
        const typeLabels = { normal: 'Con SKUs Individuales', granel: 'Granel / Unidad', agrupado: 'Producto Agrupado', bundle: 'Bundle / Combo' };
        const typeLabel = typeLabels[type] || type;

        const imgPath = d.sku_thumbnail || d.product_image || '';
        const imgHtml = imgPath 
            ? `<img src="${BASE}/${imgPath}" data-lb-src="${BASE}/${imgPath}" data-lb-caption="${esc(d.product_name)}" class="scanner-card-img" alt="Foto">`
            : `<div class="scanner-card-img-placeholder"><i class="ph ph-package"></i></div>`;

        let attributesHtml = '';
        if (d.custom_data) {
            let customObj = {};
            try { customObj = typeof d.custom_data === 'string' ? JSON.parse(d.custom_data) : d.custom_data; } catch(e) {}
            const entries = Object.entries(customObj).filter(([k, v]) => v !== null && v !== '');
            if (entries.length > 0) {
                attributesHtml = `<div class="scanner-card-attributes-grid">` + 
                    entries.map(([k, v]) => `<div class="scanner-attr-pill"><span class="scanner-attr-key">${esc(k)}:</span> <span class="scanner-attr-val">${esc(v)}</span></div>`).join('') + 
                    `</div>`;
            }
        }

        const isSku = !(d.id && d.id.toString().startsWith('bulk_'));

        card.innerHTML = `
            <div class="scanner-card-top">
                <div class="scanner-card-thumb-wrap">
                    ${imgHtml}
                </div>
                <div class="scanner-card-info-box">
                    <div class="scanner-card-tags">
                        <span class="status-badge status-${esc(status)}">${esc(status).toUpperCase()}</span>
                        <span class="scanner-type-tag"><i class="ph ph-tag"></i> ${esc(typeLabel)}</span>
                        <span class="scanner-cat-tag"><i class="ph ph-folder"></i> ${esc(d.category_name || 'General')}</span>
                    </div>
                    <h3 class="scanner-card-product-name">${esc(d.product_name)}</h3>
                    <div class="scanner-card-sku-row">
                        <span class="scanner-sku-label">CÓDIGO:</span>
                        <code class="scanner-sku-code">${esc(d.sku_code || 'SIN-CODIGO')}</code>
                    </div>
                </div>
            </div>
            ...
            <div class="scanner-card-stats-row">
                <div class="scanner-stat-box">
                    <span class="scanner-stat-label"><i class="ph ph-stack"></i> Stock / Cantidad</span>
                    <span class="scanner-stat-val">${d.is_bulk == 1 ? (d.stock || 0) + ' ' + (d.unit_type || 'Unidades') : (d.product_total_stock || 1) + ' Unidades'}</span>
                </div>
                <div class="scanner-stat-box">
                    <span class="scanner-stat-label"><i class="ph ph-currency-dollar"></i> Costo Ref.</span>
                    <span class="scanner-stat-val">S/ ${(parseFloat(d.costo_producto)||0).toFixed(2)}</span>
                </div>
                <div class="scanner-stat-box">
                    <span class="scanner-stat-label"><i class="ph ph-user"></i> Custodia / Asignado</span>
                    <span class="scanner-stat-val" style="color:${d.assigned_user_name ? '#3b82f6' : 'var(--text-muted)'};">
                        ${d.assigned_user_name ? `<i class="ph ph-user-check"></i> ${esc(d.assigned_user_name)}` : '<i class="ph ph-storefront"></i> En Almacén'}
                    </span>
                </div>
            </div>

            <div class="scanner-card-actions-row">
                <button type="button" class="btn btn-primary scanner-action-btn" onclick="openScannedItemInterfaceById('${esc(d.sku_code)}')">
                    <i class="ph ph-arrow-square-out"></i> Abrir Ficha Completa
                </button>
                ${isSku ? `
                <button type="button" class="btn btn-secondary scanner-action-btn" onclick="printSingleSkuDirect('${esc(d.sku_code)}')">
                    <i class="ph ph-printer"></i> Imprimir Etiqueta
                </button>
                <button type="button" class="btn btn-secondary scanner-action-btn" onclick="quickAssignScannedSku('${esc(d.sku_code)}')">
                    <i class="ph ph-user-plus"></i> Asignar Técnico
                </button>
                ` : ''}
            </div>
        `;
    ...
    function openScannedItemInterface(data, preferredTab = null) {
        if (!data) return;
        const autoOpen = document.getElementById('scannerAutoOpen')?.checked ?? true;
        if (!autoOpen && !preferredTab) return;

        // If it's a bulk/agrupado parent (id starts with 'bulk_')
        if (data.id && data.id.toString().startsWith('bulk_')) {
            if (typeof openBulkDetailModal === 'function') {
                openBulkDetailModal(data);
            }
        } else {
            // Individual SKU
            if (typeof openSkuDetailModal === 'function') {
                openSkuDetailModal(data);
                if (preferredTab && typeof switchDetailTab === 'function') {
                    setTimeout(() => switchDetailTab(preferredTab), 80);
                }
            }
        }
    }

    window.openScannedItemInterfaceById = function(code) {
        searchSku(code, true);
    };

    window.quickAssignScannedSku = function(code) {
        searchSku(code, true, 'assign');
    };

    window.printSingleSkuDirect = function(code) {
        if (window.showToast) window.showToast('Generando etiqueta para ' + code, 'info');
        const tabBtn = document.querySelector('.inv-toolbar-tabs .inv-tab[data-tab="etiquetas"]');
        if (tabBtn) {
            tabBtn.click();
            const labelSearch = document.getElementById('labelSearch');
            if (labelSearch) {
                labelSearch.value = code;
                labelSearch.dispatchEvent(new Event('input'));
            }
        }
    };
    ...
    function initScanner() {
        ...
        const clearBtn = document.getElementById('btnScannerClear');
        const clearHistoryBtn = document.getElementById('btnScannerClearHistory');
        let t;
        ...
        if (input) {
            input.addEventListener('input', () => {
                ...
                clearTimeout(t);
                if (val.length >= 3) {
                    t = setTimeout(() => { searchSku(val); }, 400);
                }
            });
        ...
        if (clearHistoryBtn) {
            clearHistoryBtn.addEventListener('click', () => {
                ...
                saveScannerHistory();
                renderScannerHistory();
                if (window.showToast) window.showToast('Historial de escaneos limpiado', 'info');
            });
        }

        const cameraBtn = document.getElementById('btnScanCamera');
        if (cameraBtn) {
            cameraBtn.addEventListener('click', () => {
                openSysBarcodeScanner(decoded => {
                    if (input) input.value = decoded;
                    if (clearBtn) clearBtn.style.display = 'flex';
                    searchSku(decoded, true);
                });
            });
        }
    ...
    async function searchSku(code, forceOpen = false, preferredTab = null) {
        ...
        try {
            ...
            if (res.success && res.data) {
                ...
                playScannerBeep();
                addScannerHistory(d);
                renderScannerResultCard(d);

                const autoOpen = document.getElementById('scannerAutoOpen')?.checked ?? true;
                if (forceOpen || autoOpen) {
                    openScannedItemInterface(d, preferredTab);
                }

                if (window.showToast) {
                    window.showToast(`✅ ${d.product_name} (${d.sku_code || ''}) detectado`, 'success');
                }
            } else {
                if (window.showToast) {
                    window.showToast(res.message || `No se encontró ningún producto para "${code}"`, 'error');
                }
                if (input) {
                    input.classList.add('scanner-input-shake');
                    setTimeout(() => input.classList.remove('scanner-input-shake'), 500);
                }
            }
        } catch (e) {
            console.error(e);
            if (window.showToast) window.showToast('Error de comunicación con el servidor', 'error');
        }
    }
    ...
    window.updateProdActionBar = updateProdActionBar;

    function syncFloatingBulkBar() {
        const fbb = document.getElementById('invFloatingBulkBar');
        const fbbCount = document.getElementById('fbbCount');
        if (!fbb) return;

        const prodCount = window.selectedProducts ? window.selectedProducts.size : 0;
        const skuChecks = document.querySelectorAll('.sku-row-check:checked');
        const skuCount = skuChecks.length;

        const totalSelected = prodCount > 0 ? prodCount : skuCount;

        if (totalSelected > 0) {
            if (fbbCount) fbbCount.textContent = totalSelected;
            fbb.classList.add('active');
        } else {
            fbb.classList.remove('active');
        }
    }
    window.syncFloatingBulkBar = syncFloatingBulkBar;
    ...
    function renderProductsGrid() {
        ...
        grid.innerHTML = pageData.map((p, i) => {
            ...
            const type = p.product_type || 'normal';
            return `<tr class="${rowClass}" data-product-id="${p.id}" data-product-type="${type}" data-name="${(p.name||'').toLowerCase()} ${(p.description||'').toLowerCase()} ${(p.searchable_children||'').toLowerCase()}" data-cat="${p.category_id||''}" data-disp="${p.qty_disponible||0}" data-crit="${p.stock_critico||0}" data-malo="${p.qty_malogrado||0}" data-obs="${p.qty_observacion||0}" data-total="${p.total_quantity||0}" data-inst="${p.qty_instalado||0}" style="animation: fadeIn 0.25s ease forwards; animation-delay: ${i*0.03}s; opacity: 0;">
                <td style="text-align:center;width:44px;vertical-align:middle;">
                ...
                <td data-label="Producto">
                    <div style="display:flex; align-items:center; gap:10px;">
                        ${type === 'agrupado' || type === 'bundle' ? `<button class="accordion-toggle-btn" onclick="toggleProductChildren(${p.id}, this)" title="Expandir variantes"><i class="ph ph-caret-right"></i></button>` : ''}
                        ${p.display_image 
                            ? `<img src="${BASE}/${p.display_image}" class="lb-thumb" data-lb-src="${BASE}/${p.display_image}" data-lb-caption="${esc(p.name)}" onclick="event.stopPropagation(); openImgViewer('${BASE}/${p.display_image}', '${esc(p.name).replace(/'/g, "\\'")}');" style="width:38px;height:38px;border-radius:10px;object-fit:cover;flex-shrink:0;border:1px solid var(--border-color);cursor:zoom-in;">` 
                            : (p.requires_photos == 1 
                                ? '<div style="width:38px;height:38px;border-radius:10px;background:rgba(139,92,246,0.1);display:flex;align-items:center;justify-content:center;flex-shrink:0;border:1px solid rgba(139,92,246,0.2);"><i class="ph ph-camera" style="color:#8b5cf6;font-size:1.1rem;"></i></div>'
                                : '<div style="width:38px;height:38px;border-radius:10px;background:var(--bg-color);display:flex;align-items:center;justify-content:center;flex-shrink:0;border:1px solid var(--border-color);"><i class="ph ph-package" style="color:var(--text-muted);font-size:1.1rem;opacity:0.6;"></i></div>'
                            )
                        }
                        <div>
                            <div style="font-weight:700;color:var(--text-color);font-size:0.88rem;letter-spacing:-0.2px;">${esc(p.name)}</div>
                            <div style="display:flex; gap:6px; align-items:center; flex-wrap:wrap;">
                                ${type === 'granel' || (p.is_bulk == 1 && type !== 'agrupado' && type !== 'bundle') ? '<span class="granel-badge"><i class="ph ph-scales"></i> Granel</span>' : ''}
                                ${type === 'agrupado' ? '<span class="agrupado-badge"><i class="ph ph-stack"></i> Agrupado <small>(' + (p.children_count||0) + ')</small></span>' : ''}
                                ${type === 'bundle' ? '<span class="bundle-badge"><i class="ph ph-squares-four"></i> Bundle <small>(' + (p.children_count||0) + ')</small></span>' : ''}
                            </div>
                        </div>
                    </div>
                </td>
                <td data-label="Categoría"><span class="cat-badge">${esc(p.category_name||'Sin categoría')}</span></td>
                <td data-label="Costo" class="text-end"><span style="font-weight:700;color:var(--text-color);font-size:0.85rem;">S/ ${parseFloat(p.costo_producto||0).toFixed(2)}</span></td>
                <td data-label="Total" class="text-center">
                    <span class="inv-metric-pill pill-total">${p.total_quantity}</span>
                    ${(type === 'granel' || (p.is_bulk == 1 && type !== 'agrupado' && type !== 'bundle')) ? `<small style="display:block;font-size:0.72rem;color:var(--text-muted);margin-top:2px;">${esc(p.unit_type||'Und')}</small>` : ''}
                </td>
                <td data-label="Disponibles" class="text-center"><span class="inv-metric-pill pill-disp">${p.qty_disponible}</span></td>
                <td data-label="Instalados" class="text-center"><span class="inv-metric-pill pill-inst">${p.qty_instalado}</span></td>
                <td data-label="Malogrados" class="text-center">${p.is_bulk == 1 && type !== 'agrupado' && type !== 'bundle' ? '<span class="inv-metric-pill pill-malo">0</span>' : `<span class="inv-metric-pill pill-malo">${p.qty_malogrado}</span>`}</td>
                <td data-label="Observados" class="text-center">${p.is_bulk == 1 && type !== 'agrupado' && type !== 'bundle' ? '<span class="inv-metric-pill pill-obs">0</span>' : `<span class="inv-metric-pill pill-obs">${p.qty_observacion}</span>`}</td>
                <td data-label="Acciones" class="text-center">
                    <div class="inv-actions-cell">
                        ${type === 'agrupado' || type === 'bundle' ? `<button type="button" class="inv-act-btn act-assign" onclick="openAssignGrouped(${p.id})" title="Asignar variantes / Opciones"><i class="ph ph-users-three"></i></button>` : ''}
                        ${(type !== 'granel' && p.is_bulk != 1) || (type === 'normal') ? `<button class="inv-act-btn act-skus" onclick="viewProductSkus(${p.id})" title="Ver SKUs / Códigos"><i class="ph ph-barcode"></i></button>` : ''}
                        <button class="inv-act-btn act-stock" onclick="openEditStockModal(${p.id}, '${type}', ${p.is_bulk||0}, ${p.is_bulk==1 && type!=='agrupado' && type!=='bundle' ? (p.qty_disponible||0) : (p.total_quantity||0)}, '${esc(p.unit_type||'Und')}')" title="Añadir o ajustar stock"><i class="ph ph-stack-plus"></i></button>
                        <button class="inv-act-btn act-edit" onclick="openEditProduct(${p.id}, this)" title="Editar información"><i class="ph ph-pencil-simple"></i></button>
                        <button class="inv-act-btn act-del" onclick="deleteProduct(${p.id})" title="Mover a papelera"><i class="ph ph-trash"></i></button>
                    </div>
                </td>
            </tr>`;
        }).join('');
    ...
    function renderCustomCols() {
        ...
        // Show/hide attribute scanner toolbar depending on custom columns presence
        const skuScannerCol = document.getElementById('skuScannerCol');
        if (skuScannerCol && selectedProductType === 'normal') {
            skuScannerCol.style.display = customColumns.length > 0 ? 'grid' : 'none';
        }
    ...
    function initProductModal() {
        ...
        document.getElementById('btnSaveProduct').addEventListener('click', async () => {
            ...
            const is_bulk = (selectedProductType === 'granel' || selectedProductType === 'bundle') ? 1 : 0;
            const unit_type = document.getElementById('prodUnitType') ? document.getElementById('prodUnitType').value : 'Unidades';
            const master_sku = document.getElementById('prodMasterSku') ? document.getElementById('prodMasterSku').value.trim() : '';

            if (!name) { if (window.showToast) window.showToast('El nombre es requerido', 'error'); return; }

            // Type specific validation
            if (selectedProductType === 'agrupado' || selectedProductType === 'bundle') {
                const variants = collectVariants();
                if (variants.length === 0) { if (window.showToast) window.showToast('Agrega al menos una variante u opción', 'error'); return; }
            } else if (selectedProductType === 'granel') {
                const gQty = parseInt(document.getElementById('prodGranelQty').value) || 0;
                if (gQty < 1) { if (window.showToast) window.showToast('Ingresa una cantidad de stock inicial válida', 'error'); return; }
            } else if (selectedProductType === 'normal') {
                if (quantity < 1) {
                    if (window.showToast) window.showToast('Genera o agrega al menos 1 SKU', 'error'); 
                    return;
                }
                const emptyCount = previewSkuCodes.filter(c => !c || !c.trim()).length;
                if (emptyCount > 0) {
                    if (window.showToast) window.showToast('Por favor completa todos los códigos SKU o elimina las filas vacías', 'error');
                    return;
                }
            }
            ...
            fd.append('quantity', selectedProductType === 'granel' ? (parseInt(document.getElementById('prodGranelQty').value) || 0) : quantity);
            ...
            fd.append('preview_sku_codes', JSON.stringify(previewSkuCodes));
            ...
            if (selectedProductType === 'agrupado' || selectedProductType === 'bundle') {
                ...
                // Append variant photos if bundle
                if (selectedProductType === 'bundle') {
                    variants.forEach(v => {
                        if (v.has_photo && v.file) {
                            fd.append(`variant_photo_${v.index}`, v.file);
                        }
                    });
                }
            }
            ...
            try {
                ...
                if (res.success) { 
                    ...
                    if (window.showToast) window.showToast(res.message || 'Error al guardar producto', 'error'); 
                }
            } catch (e) { 
                console.error('Error create_product:', e);
                if (window.showToast) window.showToast('Error de respuesta del servidor', 'error'); 
            }
            btn.disabled = false; btn.innerHTML = '<i class="ph ph-floppy-disk"></i> Guardar Producto';
        });
    }

    // ── Granel Live Calculation Helper ──
    window.updateGranelCalculation = function() {
        const qty = parseFloat(document.getElementById('prodGranelQty')?.value) || 0;
        const cost = parseFloat(document.getElementById('prodCosto')?.value) || 0;
        const unit = document.getElementById('prodUnitType')?.value || 'Unidades';
        
        const qtyDisplay = document.getElementById('granelValQtyDisplay');
        const costDisplay = document.getElementById('granelValCostDisplay');
        const totalDisplay = document.getElementById('granelValTotalDisplay');
        
        if (qtyDisplay) qtyDisplay.textContent = `${qty} ${unit}`;
        if (costDisplay) costDisplay.textContent = `S/ ${cost.toFixed(2)}`;
        if (totalDisplay) totalDisplay.textContent = `S/ ${(qty * cost).toFixed(2)}`;
    };

    window.setGranelQuickQty = function(qty) {
        const inp = document.getElementById('prodGranelQty');
        if (inp) {
            inp.value = qty;
            updateGranelCalculation();
        }
    };
    ...
    window.selectProductType = function(type) {
        ...
        // Reset SKU Preview & Empty State
        document.getElementById('skuPreviewWrap').style.display = 'none';
        const emptyState = document.getElementById('skuPreviewEmptyState');
        ...
        renderVariantCols();
        rebuildVariantTableHeaders(type);
        updateVariantCount();
        renderCustomCols();
        setSkuMode('auto');
        loadCategories();
        ...
        // Reset granel fields
        if (document.getElementById('prodUnitType')) document.getElementById('prodUnitType').value = 'Metros (m)';
        if (document.getElementById('prodMasterSku')) document.getElementById('prodMasterSku').value = '';
        if (document.getElementById('prodGranelQty')) document.getElementById('prodGranelQty').value = 100;
        updateGranelCalculation();
    ...
    window.applyProductTypeVisibility = function(type) {
        ...
        const agrupadoVariantsColsSection = document.getElementById('agrupadoVariantsColsSection');
        const granelRightCol = document.getElementById('granelRightCol');
        const variantsRightCol = document.getElementById('variantsRightCol');
        const badge = document.getElementById('productTypeBadge');
        const typeIcon = document.getElementById('npHeaderTypeIcon');
        const subtitle = document.getElementById('npModalSubtitle');
        const skuScannerCol = document.getElementById('skuScannerCol');

        // Update header badge and details
        const typeLabels = { normal: 'Normal', granel: 'A Granel', agrupado: 'Agrupado', bundle: 'Bundle' };
        badge.textContent = typeLabels[type] || 'Producto';
        badge.className = 'product-type-header-badge type-' + type;

        if (typeIcon) {
            if (type === 'normal') {
                typeIcon.innerHTML = '<i class="ph ph-barcode"></i>';
                typeIcon.style.background = 'linear-gradient(135deg, #3b82f6, #2563eb)';
            } else if (type === 'granel') {
                typeIcon.innerHTML = '<i class="ph ph-scales"></i>';
                typeIcon.style.background = 'linear-gradient(135deg, #f59e0b, #d97706)';
            } else if (type === 'agrupado') {
                typeIcon.innerHTML = '<i class="ph ph-stack"></i>';
                typeIcon.style.background = 'linear-gradient(135deg, #8b5cf6, #7c3aed)';
            } else if (type === 'bundle') {
                typeIcon.innerHTML = '<i class="ph ph-squares-four"></i>';
                typeIcon.style.background = 'linear-gradient(135deg, #10b981, #059669)';
            }
        }

        if (subtitle) {
            if (type === 'normal') subtitle.textContent = 'Configura los detalles del producto, personaliza atributos y gestiona los códigos SKU';
            else if (type === 'granel') subtitle.textContent = 'Gestión de materiales continuos o a granel sin seriales individuales';
            else if (type === 'agrupado') subtitle.textContent = 'Agrupa múltiples variantes bajo un producto principal con sus propios atributos';
            else if (type === 'bundle') subtitle.textContent = 'Variantes avanzadas con fotografías individuales, unidades de medida y stock propio';
        }

        // Update hidden is_bulk field
        document.getElementById('prodIsBulk').value = (type === 'granel' || type === 'bundle') ? '1' : '0';

        // Set consistent 2-column Studio Layout
        grid.style.gridTemplateColumns = 'minmax(380px, 420px) 1fr';

        if (type === 'normal') {
            if (customColsSection) customColsSection.style.display = 'block';
            if (agrupadoVariantsColsSection) agrupadoVariantsColsSection.style.display = 'none';

            if (skuRightCol) skuRightCol.style.display = 'block';
            if (skuScannerCol) skuScannerCol.style.display = customColumns.length > 0 ? 'grid' : 'none';
            if (granelRightCol) granelRightCol.style.display = 'none';
            if (variantsRightCol) variantsRightCol.style.display = 'none';

            document.getElementById('skuModeToggleWrap').style.display = 'inline-flex';
            ...
            if (skuRightCol) skuRightCol.style.display = 'none';
            if (skuScannerCol) skuScannerCol.style.display = 'none';
            if (granelRightCol) granelRightCol.style.display = 'block';
            if (variantsRightCol) variantsRightCol.style.display = 'none';

            updateGranelCalculation();

        } else if (type === 'agrupado' || type === 'bundle') {
            if (customColsSection) customColsSection.style.display = 'none';
            if (agrupadoVariantsColsSection) agrupadoVariantsColsSection.style.display = 'block';

            if (skuRightCol) skuRightCol.style.display = 'none';
            if (skuScannerCol) skuScannerCol.style.display = 'none';
            if (granelRightCol) granelRightCol.style.display = 'none';
            if (variantsRightCol) variantsRightCol.style.display = 'block';

            const wsTitle = document.getElementById('variantsWorkspaceTitle');
            if (wsTitle) {
                wsTitle.textContent = type === 'bundle' ? 'Opciones del Bundle con Fotos y Medidas' : 'Matriz de Variantes del Producto';
            }
    ...
    // ── Variant Column Management for Agrupado & Bundle ──
    ...
    window.addVariantCol = function(name) {
        ...
        renderVariantCols();
        rebuildVariantTableHeaders(selectedProductType);
        rebuildVariantTableRows();
    };
    ...
    function renderVariantCols() {
        ...
        if (variantColumns.length === 0) {
            list.innerHTML = '<div style="font-size:0.8rem; color:var(--text-muted); padding:4px 0;">Sin columnas de variante (usa las sugerencias rápidas arriba)</div>';
            return;
        }
    ...
    function rebuildVariantTableHeaders(type = selectedProductType) {
        const thead = document.getElementById('variantsTableHead');
        if (!thead) return;
        const hasCost = document.getElementById('varHasCustomCost')?.checked;
        let html = '<tr>';
        if (type === 'bundle') {
            html += '<th style="width:54px;text-align:center;"><i class="ph ph-image"></i> Foto</th>';
        }
        html += '<th style="min-width:160px;">Nombre de Variante</th>';
        variantColumns.forEach(c => { html += `<th style="min-width:100px;">${esc(c.name)}</th>`; });
        if (hasCost) html += '<th style="min-width:100px;">Costo (S/)</th>';
        if (type === 'bundle') {
            html += '<th style="min-width:110px;">U. Medida</th>';
        }
        html += '<th style="min-width:90px;text-align:center;">Cantidad</th><th style="width:50px;text-align:center;"></th></tr>';
    ...
    function rebuildVariantTableRows() {
        ...
        tbody.querySelectorAll('.variant-input-row').forEach(row => {
            const data = { 
                ...
                unit_type: row.querySelector('[data-field="unit_type"]')?.value || 'Unidades',
                attrs: {} 
            };
            const fileInput = row.querySelector('input[type="file"]');
            if (fileInput) data.fileInput = fileInput.cloneNode(true);
        ...
        tbody.innerHTML = '';
        existing.forEach(d => {
            addVariantRowWithData(d.name, d.quantity, d.costo, d.attrs, d.unit_type, d.fileInput);
        });
    }

    function addVariantRowWithData(name, quantity, costo, attrs, unit_type, existingFileInput) {
        variantCounter++;
        const hasCost = document.getElementById('varHasCustomCost')?.checked;
        ...
        // 1. Photo input for bundle
        if (type === 'bundle') {
            const tdPhoto = document.createElement('td');
            tdPhoto.style.textAlign = 'center';
            if (existingFileInput) {
                tdPhoto.appendChild(existingFileInput);
            } else {
                ...
                label.title = 'Subir foto para esta opción';
                ...
                label.appendChild(fInp);
                tdPhoto.appendChild(label);
            }
        ...
        tdName.innerHTML = `<input type="text" class="form-control" data-field="name" value="${esc(name || '')}" placeholder="Ej: Opción / Variante" style="font-size:0.85rem; font-weight:600;">`;
        ...
        variantColumns.forEach(c => {
            ...
            tdAttr.innerHTML = `<input type="text" class="form-control" data-attr="${esc(c.name)}" value="${esc(val)}" placeholder="${esc(c.name)}" style="font-size:0.85rem;">`;
        ...
        if (hasCost) {
            ...
            tdCost.innerHTML = `<input type="number" class="form-control" data-field="costo" step="0.01" min="0" value="${costo || ''}" placeholder="0.00" style="font-size:0.85rem; font-weight:700;">`;
            tr.appendChild(tdCost);
        }

        // 5. Unit Type for Bundle
        if (type === 'bundle') {
            const tdUnit = document.createElement('td');
            tdUnit.innerHTML = `<select class="form-select" data-field="unit_type" style="font-size:0.85rem;">
                <option value="Unidades" ${unit_type === 'Unidades' ? 'selected' : ''}>Unidades</option>
                <option value="Metros (m)" ${unit_type === 'Metros (m)' ? 'selected' : ''}>Metros (m)</option>
                <option value="Centímetros (cm)" ${unit_type === 'Centímetros (cm)' ? 'selected' : ''}>Centímetros (cm)</option>
                <option value="Kilogramos (kg)" ${unit_type === 'Kilogramos (kg)' ? 'selected' : ''}>Kilogramos (kg)</option>
                <option value="Gramos (g)" ${unit_type === 'Gramos (g)' ? 'selected' : ''}>Gramos (g)</option>
                <option value="Litros (L)" ${unit_type === 'Litros (L)' ? 'selected' : ''}>Litros (L)</option>
                <option value="Cajas" ${unit_type === 'Cajas' ? 'selected' : ''}>Cajas</option>
            </select>`;
        ...
        tdQty.innerHTML = `<input type="number" class="form-control" data-field="quantity" min="1" value="${quantity || 1}" style="font-size:0.88rem; font-weight:700; text-align:center;">`;
        tr.appendChild(tdQty);

        // 7. Delete
        const tdDel = document.createElement('td');
        tdDel.style.textAlign = 'center';
        tdDel.innerHTML = `<button type="button" class="btn-delete-row" onclick="removeVariantRow(this)" title="Eliminar"><i class="ph ph-trash" style="color:#ef4444;"></i></button>`;
    ...
    window.addVariantRow = function() {
        addVariantRowWithData('', '1', '', {}, 'Unidades', null);
    ...
    window.collectVariants = function() {
        ...
        rows.forEach(r => {
            ...
            const unitTypeInput = r.querySelector('[data-field="unit_type"]');
            const unitType = unitTypeInput ? unitTypeInput.value : 'Unidades';
            ...
            if (name && qty > 0) {
                ...
                const obj = { name, quantity: qty, attributes: attrs, costo: costVal, unit_type: unitType, index: index };
                
                // Photo
                const photoInput = r.querySelector('[data-field="photo"]');
                if (photoInput && photoInput.files && photoInput.files.length > 0) {
                    obj.has_photo = true;
                    obj.file = photoInput.files[0];
                }
                    ...
                    tr.innerHTML = `
                        ...
                        <td data-label="Producto">
                            <div style="display:flex; align-items:center; gap:8px; padding-left:24px;">
                                <span style="color:var(--text-muted);font-size:1.1rem;flex-shrink:0;opacity:0.5;">└</span>
                                <div>
                                    <div style="font-weight:600;color:var(--text-color);font-size:0.86rem;">${esc(child.name)}</div>
                                    <div style="display:flex;gap:4px;flex-wrap:wrap;margin-top:3px;">${attrBadges}</div>
                                </div>
                            </div>
                        </td>
                        <td data-label="Categoría"></td>
                        <td data-label="Costo" class="text-end"><span style="font-weight:700;color:var(--text-color);font-size:0.84rem;">S/ ${parseFloat(child.costo_producto||0).toFixed(2)}</span></td>
                        <td data-label="Total" class="text-center"><span class="inv-metric-pill pill-total">${qtyTotal}</span></td>
                        <td data-label="Disponibles" class="text-center"><span class="inv-metric-pill pill-disp">${qtyDisp}</span></td>
                        <td data-label="Instalados" class="text-center"><span class="inv-metric-pill pill-inst">${qtyInst}</span></td>
                        <td data-label="Malogrados" class="text-center"><span class="inv-metric-pill pill-malo">${qtyMalogrado}</span></td>
                        <td data-label="Observados" class="text-center"><span class="inv-metric-pill pill-obs">${qtyObservacion}</span></td>
                        <td data-label="Acciones" class="text-center"></td>
                    `;
    ...
    function initManualSkuScanner() {
        const input = document.getElementById('manualSkuScanInput');
        if (!input) return;
        input.addEventListener('keydown', function(e) {
            if (e.key === 'Enter') {
                ...
                addManualSkuRow(val);
                input.value = '';
                if (window.showToast) window.showToast('SKU añadido: ' + val, 'success');
            }
        });
    }
    ...
    window.setSkuMode = function(mode) {
        ...
        const autoModeWrap = document.getElementById('autoModeWrap');
        const manualModeWrap = document.getElementById('manualModeWrap');
        const npBtnAddQuickRow = document.getElementById('npBtnAddQuickRow');
        const skuScannerCol = document.getElementById('skuScannerCol');
        const emptyTitle = document.getElementById('skuEmptyStateTitle');
        const emptyDesc = document.getElementById('skuEmptyStateDesc');
        ...
        if (autoModeWrap) autoModeWrap.style.display = mode === 'auto' ? 'flex' : 'none';
        if (manualModeWrap) manualModeWrap.style.display = mode === 'manual' ? 'block' : 'none';
        if (npBtnAddQuickRow) npBtnAddQuickRow.style.display = mode === 'manual' ? 'inline-flex' : 'none';
        ...
        if (mode === 'auto') {
            if (emptyTitle) emptyTitle.textContent = 'Sin SKUs generados';
            if (emptyDesc) emptyDesc.innerHTML = 'Indica la cantidad deseada y haz clic en <strong>Generar SKUs</strong> para crear los códigos automáticamente.';
            ...
            if (emptyTitle) emptyTitle.textContent = 'Sin SKUs registrados';
            if (emptyDesc) emptyDesc.innerHTML = 'Haz clic en <strong>Agregar Fila Manual</strong> o escanea códigos con tu lector láser para añadirlos a la lista.';
            if (previewSkuCodes.length === 0) {
                document.getElementById('skuPreviewWrap').style.display = 'none';
                const emptyState = document.getElementById('skuPreviewEmptyState');
                if (emptyState) emptyState.style.display = 'flex';
            ...
            const manualScanInput = document.getElementById('manualSkuScanInput');
            if (manualScanInput) setTimeout(() => manualScanInput.focus(), 100);
        }
    };
    ...
    window.addManualSkuRow = function(code = '') {
        const trimmed = (typeof code === 'string') ? code.trim() : '';
        previewSkuCodes.push(trimmed);
        const obj = {}; customColumns.forEach(c => obj[colObj(c).name] = ''); previewCustomData.push(obj);
        renderPreviewSkuTable();
        if (!trimmed) {
            setTimeout(() => {
    ...
    function renderPreviewSkuTable() {
        ...
        const npBtnAddQuickRow = document.getElementById('npBtnAddQuickRow');
        if (npBtnAddQuickRow) npBtnAddQuickRow.style.display = skuMode === 'manual' ? 'inline-flex' : 'none';
        ...
        headHtml += '<th style="text-align:center;width:40px;"></th></tr>';
        ...
        for (let i = 0; i < qty; i++) {
            ...
            if (requiresPhotos) {
                if (firstProductThumb) {
                    ...
                    rowsHtml += `<td><div title="Sin foto - agrégala en la sección de fotos" style="width:34px;height:34px;border-radius:8px;background:var(--bg-color);border:1.5px dashed var(--border-color);display:flex;align-items:center;justify-content:center;"><i class="ph ph-camera" style="color:var(--text-muted);font-size:0.9rem;opacity:0.45;"></i></div></td>`;
                }
            }
            ...
            rowsHtml += `<td data-label="" style="text-align:center;"><button type="button" class="btn-delete-row" onclick="deleteSkuRow(${i})" title="Eliminar"><i class="ph ph-trash"></i></button></td></tr>`;
        }
    ...
    window.updateContinuousScanProgress = function() {
        ...
        if (!select || !countEl || !barEl || !textEl) return;
        ...
        if (total === 0) {
            ...
            barEl.style.width = '0%';
            textEl.textContent = 'Genera SKUs para comenzar';
        ...
        if (!col) {
            ...
            barEl.style.width = '0%';
            textEl.textContent = 'Selecciona una columna destino';
            textEl.style.color = 'var(--text-muted)';
            barEl.style.background = 'var(--primary-color)';
            return;
        }
        ...
        if (scanned === total) {
            textEl.textContent = '¡Completado todos los SKUs!';
            textEl.style.color = '#10b981';
            barEl.style.background = '#10b981';
        } else {
            textEl.textContent = `Faltan ${total - scanned} por escanear (${pct}%)`;
            textEl.style.color = 'var(--text-muted)';
            barEl.style.background = 'var(--primary-color)';
        }
    };
    ...
    // ── Labels Tab: State & Mode Handlers ──
    window.labelSourceMode = 'batch'; // 'batch' or 'reprint'
    window.reprintQueue = []; // array of { id, sku_code, product_name, category_name }
    window.currentLabelDateMode = 'all';
    window.lastGeneratedSkuIds = [];

    window.setLabelPrintSourceMode = function(mode, btn) {
        window.labelSourceMode = mode;
        document.querySelectorAll('.lbl-mode-tab').forEach(t => t.classList.remove('active'));
        if (btn) btn.classList.add('active');

        const batchWrap = document.getElementById('lblSourceModeBatchWrap');
        const reprintWrap = document.getElementById('lblSourceModeReprintWrap');

        if (mode === 'reprint') {
            if (batchWrap) batchWrap.style.display = 'none';
            if (reprintWrap) reprintWrap.style.display = 'block';
            setTimeout(() => document.getElementById('reprintSkuInput')?.focus(), 100);
        } else {
            if (batchWrap) batchWrap.style.display = 'block';
            if (reprintWrap) reprintWrap.style.display = 'none';
        }
    };

    window.setLabelDateMode = function(mode, btn) {
        window.currentLabelDateMode = mode;
        document.querySelectorAll('.lbl-date-chip').forEach(c => c.classList.remove('active'));
        if (btn) btn.classList.add('active');

        const singleWrap = document.getElementById('lblDateSingleWrap');
        const rangeWrap = document.getElementById('lblDateRangeWrap');
    ...
    // ── Reprint Queue Methods ──
    window.renderReprintQueue = function() {
        const list = document.getElementById('reprintQueueList');
        const countBadge = document.getElementById('reprintCount');
        if (countBadge) countBadge.textContent = window.reprintQueue.length;

        if (!list) return;
        if (window.reprintQueue.length === 0) {
            list.innerHTML = `<div class="reprint-empty-hint">
        ...
        list.innerHTML = window.reprintQueue.map((item, idx) => `
            <div class="reprint-queue-item">
    ...
    window.addSkuToReprint = function(sku) {
        ...
        if (!window.reprintQueue.some(x => x.id === sku.id)) {
            ...
            if (window.showToast) window.showToast(`SKU ${sku.sku_code} agregado para reimpresión`, 'success');
        } else {
            if (window.showToast) window.showToast(`El SKU ${sku.sku_code} ya está en la lista`, 'info');
        }
        const dropdown = document.getElementById('reprintSearchResults');
        if (dropdown) dropdown.style.display = 'none';
        const inp = document.getElementById('reprintSkuInput');
    ...
    window.clearReprintQueue = function() {
        window.reprintQueue = [];
        window.renderReprintQueue();
    };

    window.quickAddReprintInput = async function() {
        const inp = document.getElementById('reprintSkuInput');
        const q = inp ? inp.value.trim() : '';
        if (!q) return;

        const fd = new FormData();
        fd.append('action', 'search_skus_for_labels');
        fd.append('query', q);
                ...
                try {
                    ...
                    if (res.success && res.data && res.data.length > 0) {
                        dropdown.innerHTML = res.data.map(item => `
                            <div class="reprint-result-item" onclick='addSkuToReprint(${JSON.stringify(item)})'>
                                <div class="reprint-res-left">
                                    <span class="reprint-res-sku">${esc(item.sku_code)}</span>
                                    <span class="reprint-res-prod">${esc(item.product_name)}</span>
                                </div>
                                <span style="font-size:0.72rem; color:var(--text-muted);">${item.status || 'disponible'}</span>
                            </div>
                        `).join('');
    ...
    async function generateLabels() {
        ...
        const labelType = document.getElementById('labelType')?.value || 'barcode';
        ...
        const showName = document.getElementById('labelShowName') ? document.getElementById('labelShowName').checked : true;
        const showSku = document.getElementById('labelShowSku') ? document.getElementById('labelShowSku').checked : true;
        const showDate = document.getElementById('labelShowDate') ? document.getElementById('labelShowDate').checked : false;

        let skusToRender = [];

        if (window.labelSourceMode === 'reprint') {
            // Reimpresión de SKUs específicos
            if (!window.reprintQueue || window.reprintQueue.length === 0) {
                if (window.showToast) window.showToast('Agrega al menos un SKU a la lista de reimpresión', 'warning');
                return;
            }
            ...
            fd.append('action', 'get_product_skus'); 
            fd.append('product_id', productId);
            if (printStatus !== '') fd.append('print_status', printStatus);

            const mode = window.currentLabelDateMode || 'all';
            if (mode === 'today') {
                const todayStr = new Date().toISOString().split('T')[0];
                fd.append('start_date', todayStr);
                fd.append('end_date', todayStr);
            } else if (mode === 'single') {
                const sDate = document.getElementById('labelSingleDate')?.value;
                if (sDate) fd.append('start_date', sDate);
            } else if (mode === 'range') {
                const rStart = document.getElementById('labelStartDate')?.value;
                const rEnd = document.getElementById('labelEndDate')?.value;
                if (rStart) fd.append('start_date', rStart);
                if (rEnd) fd.append('end_date', rEnd);
            }

            const res = await fetch(BASE + '/ajax/inventario.php', { method: 'POST', body: fd }).then(r => r.json());
            if (!res.success || !res.data.length) { 
                preview.innerHTML = '<div class="labels-empty-placeholder"><div class="lep-icon" style="background:rgba(239,68,68,0.1);color:#ef4444;border-color:rgba(239,68,68,0.25);"><i class="ph ph-tag"></i></div><h4>Sin SKUs coincidentes</h4><p>No se encontraron SKUs para este producto con los filtros y fechas seleccionadas.</p></div>'; 
                const metaEl = document.getElementById('labelPreviewMeta');
                if (metaEl) metaEl.innerHTML = '<span style="font-size:0.8rem; color:#ef4444;">0 etiquetas</span>';
        ...
        const qrSize = Math.max(28, Math.min(65, Math.min(labelW, labelH) * 1.2));

        window.lastGeneratedSkuIds = [];

        skusToRender.forEach(s => {
            ...
            for (let c = 0; c < labelCopies; c++) {
                ...
                if (showDate && s.created_at) {
                    const onlyDate = s.created_at.split(' ')[0];
                    const parts = onlyDate.split('-');
                    const formattedDate = parts.length === 3 ? `${parts[2]}/${parts[1]}/${parts[0]}` : onlyDate;
                    dateHtml = `<div class="label-date-text">Reg: ${formattedDate}</div>`;
                }

                if (labelType === 'barcode') {
                    html += `<div class="label-item">${logoHtml}${companyHtml}${nameHtml}<svg id="${id}"></svg>${skuHtml}${dateHtml}</div>`;
                } else {
                    html += `<div class="label-item">${logoHtml}${companyHtml}${nameHtml}<div id="${id}" style="margin:0 auto; display:flex; justify-content:center; align-items:center;"></div>${skuHtml}${dateHtml}</div>`;
                }
            }
        });
        html += '</div>';
        ...
        // Count info
        const totalLabels = skusToRender.length * labelCopies;
        const totalRows = Math.ceil(totalLabels / labelCols);
        html += `<div style="text-align:center; margin-top:14px; font-size:0.82rem; color:var(--text-muted);">
        ...
        const metaEl = document.getElementById('labelPreviewMeta');
        if (metaEl) {
            metaEl.innerHTML = `<span style="font-size:0.8rem; font-weight:700; color:#10b981; display:flex; align-items:center; gap:4px;"><i class="ph ph-check-circle"></i> ${totalLabels} etiquetas generadas</span>`;
        }
                    ...
                    if (labelType === 'barcode') {
                        ...
                        try {
                            ...
                            if (typeof QRCode !== 'undefined') {
                                ...
                                // Fallback if QRCode CDN is blocked: QR SVG via API
                                el.innerHTML = `<img src="https://api.qrserver.com/v1/create-qr-code/?size=${Math.round(qrSize)}x${Math.round(qrSize)}&data=${encodeURIComponent(s.sku_code)}" style="width:${Math.round(qrSize)}px;height:${Math.round(qrSize)}px;" alt="QR">`;
                            }
                        } catch(e) { 
...
<body>
    ...
    window.copyModalSkuCode = function() {
        if (!currentSkuData || !currentSkuData.sku_code) return;
        navigator.clipboard.writeText(currentSkuData.sku_code).then(() => {
            if (window.showToast) window.showToast(`SKU ${currentSkuData.sku_code} copiado al portapapeles`, 'success');
        });
    };

    window.printModalSkuLabel = function() {
        if (!currentSkuData) return;
        window.addSkuToReprint({
            id: currentSkuData.id,
            sku_code: currentSkuData.sku_code,
            product_name: currentSkuData.product_name,
            category_name: currentSkuData.category_name
        });
        window.closeSkuDetail();
        const tabEtiquetasBtn = document.querySelector('.inv-tab[data-tab="etiquetas"]');
        if (tabEtiquetasBtn) {
            tabEtiquetasBtn.click();
            window.setLabelPrintSourceMode('reprint', document.getElementById('lblTabModeReprint'));
            setTimeout(() => document.getElementById('btnGenLabels')?.click(), 150);
        }
    };
    ...
    function openSkuDetailModal(data) {
        ...
        if (titleEl) titleEl.textContent = data.product_name || 'Detalle del Activo';
        
        const skuTextEl = document.getElementById('sdhSkuText');
        if (skuTextEl) skuTextEl.textContent = data.sku_code || '—';
        
        const catTextEl = document.getElementById('sdhCatText');
        if (catTextEl) catTextEl.textContent = data.category_name || 'General';

        const statusPill = document.getElementById('sdhStatusPill');
        const statusText = document.getElementById('sdhStatusText');
        if (statusPill && statusText) {
            statusText.textContent = status.toUpperCase();
            const bgMap = {
                disponible: 'rgba(16,185,129,0.15)',
                instalado: 'rgba(59,130,246,0.15)',
                malogrado: 'rgba(239,68,68,0.15)',
                reparado: 'rgba(245,158,11,0.15)',
                en_transito: 'rgba(139,92,246,0.15)',
                observacion: 'rgba(8,145,178,0.15)'
            };
            statusPill.style.background = bgMap[status] || 'rgba(99,102,241,0.15)';
            statusPill.style.color = col;
            statusPill.style.border = `1px solid ${col}44`;
        }

        // ── Imagen del producto en el header ──
        const headerImg = document.getElementById('skuDetailHeaderImg');
        const thumbSrc = data.sku_thumbnail || data.product_image;
        if (headerImg) {
            if (thumbSrc) {
                ...
                headerImg.innerHTML = `<i class="ph ph-package" style="font-size:1.8rem;color:${col};"></i>`;
                headerImg.style.background = col + '15';
                headerImg.style.border = '2px solid ' + col + '44';
            }
        }
        ...
        const statusSelect = document.getElementById('skuDetailStatus');
        ...
        if (data.product_type === 'agrupado') {
            bulkExtra = `
                <div class="sku-spec-card">
                    <div class="ssc-label"><i class="ph ph-stack" style="color:#8b5cf6;"></i> Tipo</div>
                    <div class="ssc-val" style="color:#8b5cf6;">Agrupado</div>
                </div>
                <div class="sku-spec-card">
                    <div class="ssc-label"><i class="ph ph-chart-bar" style="color:#8b5cf6;"></i> Stock Total</div>
                    <div class="ssc-val" style="color:#8b5cf6;">${data.stock} ${data.unit_type||'Uds'}</div>
                </div>`;
        } else if (data.is_bulk == 1) {
            bulkExtra = `
                <div class="sku-spec-card">
                    <div class="ssc-label"><i class="ph ph-cube" style="color:#3b82f6;"></i> Stock Almacén</div>
                    <div class="ssc-val" style="color:#3b82f6;">${data.stock} ${data.unit_type||''}</div>
                </div>`;
        }
        ...
        try {
            ...
            if (Object.keys(cd).length > 0) {
                for (const [k, v] of Object.entries(cd)) {
                    customColsHtml += `
                        <div class="sku-spec-card">
                            <div class="ssc-label"><i class="ph ph-hash" style="color:#06b6d4;"></i> ${esc(k)}</div>
                            <div class="ssc-val" style="font-family:monospace;">${esc(v || '—')}</div>
                        </div>`;
                }
            } else if (data.product_description) {
                customColsHtml = `
                    <div class="sku-spec-card" style="grid-column: span 2;">
                        <div class="ssc-label"><i class="ph ph-text-align-left"></i> Descripción</div>
                        <div class="ssc-val" style="font-weight:normal;font-size:0.85rem;">${esc(data.product_description)}</div>
                    </div>`;
            }
        } catch(e) {}
        ...
        info.innerHTML = `
            <div class="sku-spec-card">
                <div class="ssc-label"><i class="ph ph-barcode" style="color:#6366f1;"></i> SKU Code</div>
                <div class="ssc-val" style="font-family:monospace; color:#6366f1;">${esc(data.sku_code)}</div>
            </div>
            <div class="sku-spec-card">
                <div class="ssc-label"><i class="ph ph-package" style="color:#3b82f6;"></i> Producto</div>
                <div class="ssc-val text-truncate" title="${esc(data.product_name)}">${esc(data.product_name)}</div>
            </div>
            <div class="sku-spec-card">
                <div class="ssc-label"><i class="ph ph-folder" style="color:#f59e0b;"></i> Categoría</div>
                <div class="ssc-val">${esc(data.category_name || 'Sin categoría')}</div>
            </div>
            ${data.is_bulk == 1 ? '' : `
            <div class="sku-spec-card">
                <div class="ssc-label"><i class="ph ph-activity" style="color:${col};"></i> Estado</div>
                <div class="ssc-val" style="color:${col};">${esc(status).toUpperCase()}</div>
            </div>`}
            ${data.is_bulk == 1 ? '' : `
            <div class="sku-spec-card">
                <div class="ssc-label"><i class="ph ph-user" style="color:#8b5cf6;"></i> Asignado a</div>
                <div class="ssc-val text-truncate">${data.assigned_user_name ? esc(data.assigned_user_name) : '<span style="color:var(--text-muted);font-weight:normal;">Sin asignar</span>'}</div>
            </div>`}
            ${bulkExtra}
            ${customColsHtml}`;
...
(function() {
    ...
    window.updateSkuSelection = function() {
        ...
        const actBar = document.getElementById('skuActiveActions');
        if (actBar) actBar.style.display = checked > 0 ? 'flex' : 'none';
        ...
        if (window.syncFloatingBulkBar) window.syncFloatingBulkBar();
    };

    window.clearCurrentSelection = function() {
        if (window.selectedProducts) window.selectedProducts.clear();
        document.querySelectorAll('.prod-row-check').forEach(c => {
            c.checked = false;
            c.closest('tr')?.classList.remove('row-selected');
        });
        const pCheckAll = document.getElementById('prodCheckAll');
        if (pCheckAll) pCheckAll.checked = false;
        if (window.updateProdActionBar) window.updateProdActionBar();

        document.querySelectorAll('.sku-row-check').forEach(c => c.checked = false);
        const sCheckAll = document.getElementById('skuCheckAll');
        if (sCheckAll) sCheckAll.checked = false;
        updateSkuSelection();
    ...
    window.openBulkAssignModal = async function() {
        const prodCount = window.selectedProducts ? window.selectedProducts.size : 0;
        const skuChecks = Array.from(document.querySelectorAll('.sku-row-check:checked'));
        const totalCount = prodCount > 0 ? prodCount : skuChecks.length;

        if (totalCount === 0) {
            if (window.showToast) window.showToast('No hay elementos seleccionados', 'warning');
            return;
        }

        const countElem = document.getElementById('bulkAssignItemCount');
        ...
        try {
            ...
            if (res.success && res.data) {
                const sel = document.getElementById('bulkAssignUserId');
                if (sel) {
                    sel.innerHTML = '<option value="">-- Selecciona el técnico --</option>' +
                        res.data.map(u => `<option value="${u.id}">${u.name} (${u.role})</option>`).join('');
                }
            }
        } catch(e) {}

        const modalElem = document.getElementById('modalBulkAssign');
        if (modalElem) {
    ...
    window.submitBulkAssign = async function(e) {
        e.preventDefault();
        const btn = document.getElementById('btnSubmitBulkAssign');
        btn.disabled = true;
        btn.innerHTML = `<i class="ph ph-spinner ph-spin"></i> Asignando...`;

        const userId = document.getElementById('bulkAssignUserId').value;
        const isEpp = document.getElementById('bulkAssignIsEpp').checked ? 1 : 0;
        ...
        try {
            let items = [];
            if (prodCount > 0) {
                items = Array.from(window.selectedProducts).map(id => 'bulk_' + id);
            } else {
                items = skuChecks.map(c => c.value);
            }

            const fd = new FormData();
            fd.append('action', 'bulk_assign_skus');
            fd.append('user_id', userId);
            fd.append('is_epp', isEpp);
            fd.append('notes', notes);
            fd.append('skus', JSON.stringify(items));
            if (window.CSRF_TOKEN) fd.append('csrf_token', window.CSRF_TOKEN);

            const res = await fetch(BASE + '/ajax/inventario.php', {
                method: 'POST',
                body: fd
            }).then(r => r.json());

            if (res.success) {
                if (window.showToast) window.showToast(res.message || 'Asignación completada', 'success');
                window.closeBulkAssignModal();

                window.clearCurrentSelection();
                if (window.loadAllSkus) window.loadAllSkus();
                if (window.loadProducts) window.loadProducts();
                if (window.loadMetrics) window.loadMetrics();
            } else {
            ...
            console.error(err);
            alert('Error de conexión al procesar la asignación');
        } finally {
            btn.disabled = false;
            btn.innerHTML = `<i class="ph-bold ph-check"></i> Asignar Ahora`;
        }
    };

    window.executeBulkUnassign = async function() {
        const skuChecks = Array.from(document.querySelectorAll('.sku-row-check:checked'));
        if (!skuChecks.length) {
            if (window.showToast) window.showToast('Selecciona los SKUs que deseas desasignar', 'warning');
            return;
        }

        if (!confirm(`¿Desasignar los ${skuChecks.length} SKU(s) seleccionados y devolverlos al almacén central?`)) return;

        try {
            const items = skuChecks.map(c => c.value);
            const fd = new FormData();
            fd.append('action', 'bulk_unassign_skus');
            fd.append('skus', JSON.stringify(items));
            if (window.CSRF_TOKEN) fd.append('csrf_token', window.CSRF_TOKEN);
    ...
    window.openBulkStatusModal = function() {
        const skuChecks = Array.from(document.querySelectorAll('.sku-row-check:checked'));
        if (!skuChecks.length) {
            if (window.showToast) window.showToast('Selecciona al menos un SKU para cambiar su estado', 'warning');
            return;
        }

        const countElem = document.getElementById('bulkStatusItemCount');
        if (countElem) countElem.textContent = skuChecks.length;

        const modalElem = document.getElementById('modalBulkStatus');
        if (modalElem) {
            modalElem.classList.add('active');
        }
    };
    ...
    window.submitBulkStatus = async function(e) {
        ...
        try {
            ...
            const res = await fetch(BASE + '/ajax/inventario.php', { method: 'POST', body: fd }).then(r => r.json());
            if (res.success) {
                if (window.showToast) window.showToast(res.message || 'Estados actualizados', 'success');
            ...
            btn.disabled = false;
            btn.innerHTML = `<i class="ph-bold ph-check"></i> Aplicar Cambio`;
        }
    };
    ...
    window.exportCurrentBulkSelection = function() {
        const prodCount = window.selectedProducts ? window.selectedProducts.size : 0;
        if (prodCount > 0) {
