...
// Scanner state for off-canvas
let bpScanBuffer = '';
let bpScanTimeout = null;

// ── Init ──────────────────────────────────────────
document.addEventListener('DOMContentLoaded', () => {
    loadStats();
    loadUsers();
    // Global listener para escáner
    document.addEventListener('keydown', handleBackpackScan);
});

async function handleBackpackScan(e) {
    // Solo si el offcanvas está activo y no estamos escribiendo en un input
    const oc = document.getElementById('mochilaOffCanvas');
    if (!oc || !oc.classList.contains('active') || !currentUserId) return;
    ...
    if (e.key === 'Enter') {
        e.preventDefault();
        const code = bpScanBuffer.trim();
        bpScanBuffer = '';
        if (code) {
            await assignScannedSkuToBackpack(code);
        }
        return;
    }
    ...
    if (e.key.length === 1) {
        bpScanBuffer += e.key;
        clearTimeout(bpScanTimeout);
        bpScanTimeout = setTimeout(() => {
            bpScanBuffer = '';
        }, 100);
    }
}

async function assignScannedSkuToBackpack(code) {
    if (window.showToast) window.showToast('Asignando ' + code + '...', 'info');
    
    try {
        const fd = new FormData();
        fd.append('action', 'assign_sku_by_code');
        fd.append('sku_code', code);
        fd.append('target_user_id', currentUserId);

        const res = await fetch(`${BASE}/ajax/mochila.php`, { method: 'POST', body: fd });
        const data = await res.json();
        
        if (data.success) {
            if (window.showToast) window.showToast(data.message || 'Asignado a mochila', 'success');
            // Recargar mochila y stats
            loadOffCanvasBackpack(currentUserId);
            loadUsers();
            loadStats();
        } else {
            if (window.showToast) window.showToast(data.message || 'Error al asignar', 'error');
        }
    } catch (e) {
        if (window.showToast) window.showToast('Error de red al asignar', 'error');
    }
}
