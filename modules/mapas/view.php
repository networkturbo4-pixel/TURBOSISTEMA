...
<style>
    ...
    .tool-btn:hover { background: rgba(255, 255, 255, 0.1); color: white; transform: translateY(-1px); }
    .tool-btn.active { background: #38bdf8; color: #0f172a; box-shadow: 0 4px 10px rgba(56, 189, 248, 0.3); }
    
    .tool-btn-back { background: rgba(56, 189, 248, 0.1); color: #38bdf8; }

    .toolbar-divider { width: 1px; height: 24px; background: rgba(255,255,255,0.2); margin: 0 4px; align-self: center; }
...
function initMap() {
    map = new google.maps.Map(document.getElementById("map"), {
        ...
        zoom: 15,
        mapTypeId: google.maps.MapTypeId.HYBRID, // Satélite con calles
    ...
    google.maps.event.addListener(drawingManager, 'overlaycomplete', function(event) {
        ...
        if(tipo === 'LineString') {
            ...
            // Re-calcular si el usuario edita la línea
            ...
            let path = currentDrawing.getPath().getArray().map(p => [p.lng(), p.lat()]);
            path.push(path[0]); // Cerrar polígono
...
function toggleMeasure(btn) {
    // La medidón se maneja usando draw_line_string que ahora abre el panel Earth
...
function renderMarkers() {
    ...
    currentGeoJson.features.forEach(f => {
        if (f.geometry.type === 'Point') {
            ...
            // Usar AdvancedMarkerElement si es posible, o Marker clásico
            let iconUrl;
            if (icono.includes('/')) {
