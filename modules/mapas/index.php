...
<div class="container-fluid fibra-container">
            ...
            <div class="central-map-controls-bar">
                <div class="map-control-group">
                    ...
                    <button type="button" class="map-type-btn active" onclick="setGlobalMapType('hybrid')">
                        <i class="ph-bold ph-planet"></i> Satelital / HÃƒÂ­brido
                    </button>
    ...
    <div class="fibra-tab-content" id="tab-gps">
        <div class="gps-header-card">
            <div>
                <h3 style="margin:0 0 4px; font-size:1.15rem; font-weight:800; color:var(--text-color);">GeolocalizaciÃƒÂ³n y Registro de Nodos</h3>
            ...
            <div style="display:flex; gap:10px; flex-wrap:wrap;">
                <button type="button" class="gps-loc-btn" onclick="detectLiveLocation()">
                    <i class="ph-bold ph-crosshair"></i> Mi UbicaciÃƒÂ³n GPS en Vivo
                </button>
            </div>
        </div>
    ...
    <div class="fibra-tab-content" id="tab-archivos">
        <div class="files-upload-zone" id="filesUploadZone" onclick="document.getElementById('kmlFileInput').click()">
            ...
            <p style="margin:0; font-size:0.85rem; color:var(--text-muted); max-width:500px; margin:0 auto;">
                Arrastra y suelta aquÃƒÂ­ tus archivos exportados de Google Earth, MyMaps o QGIS para crear una nueva zona de fibra automÃƒÂ¡ticamente.
            </p>
        </div>

        <div class="files-grid-card">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:16px; flex-wrap:wrap; gap:10px;">
                <h4 style="margin:0; font-size:1.05rem; font-weight:800; color:var(--text-color);">ExportaciÃƒÂ³n y Formatos Soportados</h4>
            ...
            <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(240px, 1fr)); gap:14px;">
                <div style="background:rgba(255,255,255,0.03); border:1px solid var(--border-color); border-radius:12px; padding:16px;">
                    ...
                    <p style="margin:0; font-size:0.8rem; color:var(--text-muted);">Soporta puntos de cajas NAP, mufas, polÃƒÂ­gonos de zonas y lÃƒÂ­neas de trazado de cable.</p>
                </div>
                <div style="background:rgba(255,255,255,0.03); border:1px solid var(--border-color); border-radius:12px; padding:16px;">
                    <div style="display:flex; align-items:center; gap:10px; margin-bottom:8px;">
                        <span class="badge" style="background:#10b981; color:#fff; font-size:0.75rem;">GeoJSON</span>
                        <strong style="font-size:0.9rem;">EstÃƒÂ¡ndar GIS / Web</strong>
                    </div>
...
<div class="edit-modal-overlay" id="editModalOverlay">
    <div class="edit-modal-content">
        ...
        <textarea id="editProjectDescInput" placeholder="DescripciÃƒÂ³n de la zona de cobertura (opcional)" rows="3"></textarea>
...
async function loadProjects() {
    try {
        ...
        if (res.success && res.data && res.data.length > 0) {
            ...
            allProjectsData = [];
            let msg = currentZonasFilter === 'archivadas' ? "No tienes mapas archivados" : "No tienes mapas de zonas todavÃƒÂ­a";
            grid.innerHTML = `
                <div style="text-align:center; grid-column: 1 / -1; padding: 60px; color: var(--text-muted); background: var(--surface-color); border-radius: 16px; border: 1px dashed var(--border-color);">
                    ...
                    <p style="margin:0 0 16px;">${currentZonasFilter === 'archivadas' ? 'Las zonas que archives aparecerÃƒÂ¡n aquÃƒÂ­.' : 'Crea tu primera zona de fibra para empezar a mapear la red.'}</p>
...
function renderGpsTable(nodes) {
    ...
    if (nodes.length === 0) {
        tbody.innerHTML = `<tr><td colspan="7" style="text-align:center; padding:30px; color:var(--text-muted);">No hay nodos georreferenciados todavÃƒÂ­a.</td></tr>`;
        return;
    }
...
function detectLiveLocation() {
    if (!navigator.geolocation) {
        alert('La geolocalizaciÃƒÂ³n no estÃƒÂ¡ soportada en tu navegador');
        return;
    }
    navigator.geolocation.getCurrentPosition(
        pos => {
            ...
            if (window.showToast) window.showToast(`Ã°Å¸â€œÂ UbicaciÃƒÂ³n encontrada: ${lat.toFixed(5)}, ${lng.toFixed(5)}`, 'success');
        },
        err => {
            alert('No se pudo obtener tu ubicaciÃƒÂ³n GPS. Verifica los permisos.');
        },
    ...
    reader.onload = async function(evt) {
        try {
            ...
            if (!geojsonData || !geojsonData.features || geojsonData.features.length === 0) {
                alert('El archivo no contiene elementos geogrÃƒÂ¡ficos vÃƒÂ¡lidos');
                return;
            }

            const projectName = file.name.replace(/\.[^/.]+$/, "");
            if (!confirm(`Se detectaron ${geojsonData.features.length} elementos en "${file.name}". Ã‚Â¿Deseas crear la zona "${projectName}" e importar los elementos?`)) {
                return;
            }
            ...
            if (resImp.success) {
                alert(`Ã¢Å“â€¦ ${resImp.message}. Redirigiendo al mapa...`);
...
function archiveProject(id, e) {
    ...
    if(!confirm('Ã‚Â¿EstÃƒÂ¡s seguro de archivar este mapa de zona?')) return;
...
function unarchiveProject(id, e) {
    e.stopPropagation();
    document.getElementById('menu-'+id)?.classList.remove('show');
    if(!confirm('Ã‚Â¿Deseas desarchivar este mapa y devolverlo a la lista de zonas activas?')) return;
...
function deleteProject(id, e) {
    e.stopPropagation();
    document.getElementById('menu-'+id)?.classList.remove('show');
    if(!confirm('Ã‚Â¡ATENCIÃƒâ€œN! Ã‚Â¿EstÃƒÂ¡s totalmente seguro de eliminar este mapa? Se borrarÃƒÂ¡n todos los puntos, hilos y fotos. Esto no se puede deshacer.')) return;
