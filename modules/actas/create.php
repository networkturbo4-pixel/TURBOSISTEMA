...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (localStorage.getItem(SW_KEY_START)) {
        // Active localStorage chronometer — resume it
        const tInicio = document.getElementById('srvHoraInicio');
        const savedInicio = localStorage.getItem(SW_KEY_INICIO);
        if (savedInicio) tInicio.value = savedInicio;
        setButtonState('running');
        updateStopwatchDisplay();
        stopwatchInterval = setInterval(updateStopwatchDisplay, 1000);
    } else {
        // No localStorage — try to restore from DB values (only works if fields are already populated)
    ...
    window.restoreStopwatchFromDB = function() {
        ...
        if (isRealTime(horaInicio) && isRealTime(horaFin)) {
            // Both times set — show as "Finalizado" with Reiniciar available
            ...
            // Hora inicio set but no fin — service is still running, resume from DB
            const srvFecha = document.querySelector('input[name="srv_fecha"]').value;
