...
$system_modules = [
    ...
    'inventario' => [
        ...
        'submodules' => [
            ...
            'historial' => [
                'name' => 'Historial',
                'url' => '/modules/inventario/historial',
            ],
            'activos' => [
                'name' => 'Activos Empresariales',
    ...
    'sistema' => [
        ...
        'icon' => 'ph-hard-drives',
        'url' => '/modules/sistema',
        'description' => 'Copias de seguridad y actualización del sistema desde GitHub',
        'default_access' => false
    ]
];
...
function requirePermission($pdo, $module_key) {
    if (!hasAccess($pdo, $module_key)) {
        if (strpos($_SERVER['REQUEST_URI'] ?? '', '/ajax/') !== false || (isset($_SERVER['HTTP_ACCEPT']) && strpos($_SERVER['HTTP_ACCEPT'], 'application/json') !== false)) {
            ...
            echo json_encode(['success' => false, 'message' => 'Acceso denegado: No tienes permisos para el módulo ' . $module_key]);
            exit;
        }
