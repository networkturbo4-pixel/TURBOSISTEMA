...
let webcamCameraMode = 'foto';
let webcamMediaRecorder = null;
let webcamRecordedChunks = [];
let webcamRecordingTimer = null;
let webcamRecordingSeconds = 0;
...
async function flipCamera() {
    ...
    const video = document.getElementById('webcamVideo');
    const needsAudio = (webcamCameraMode === 'video');
...
async function setCameraMode(mode) {
    ...
    if (mode === 'foto') {
        ...
        shutterInner.style.width = '58px';
        shutterInner.style.height = '58px';
        btnShutter.onclick = toggleWebcamRecording;
...
function confirmCapture() {
    ...
    const isVideo = webcamCapturedBlob.type.startsWith('video');
    const ext = isVideo ? 'mp4' : 'jpg';
    const mimeType = isVideo ? 'video/mp4' : 'image/jpeg';
    const capturedFile = new File([webcamCapturedBlob], `cam_${Date.now()}.${ext}`, { type: mimeType });
...
function startWebcamRecording() {
    ...
    webcamMediaRecorder.ondataavailable = (e) => {
        if (e.data && e.data.size > 0) webcamRecordedChunks.push(e.data);
    };
    
    webcamMediaRecorder.onstop = () => {
        const mimeUsed = webcamMediaRecorder ? (webcamMediaRecorder.mimeType || 'video/webm') : 'video/webm';
        const blob = new Blob(webcamRecordedChunks, { type: mimeUsed });
    ...
    webcamRecordingSeconds = 0;
    updateWebcamRecordingTimer();
    webcamRecordingTimer = setInterval(updateWebcamRecordingTimer, 1000);
...
function stopWebcamRecording() {
    ...
    if (webcamRecordingTimer) {
        clearInterval(webcamRecordingTimer);
        webcamRecordingTimer = null;
    }
...
function updateWebcamRecordingTimer() {
    webcamRecordingSeconds++;
    const mins = String(Math.floor(webcamRecordingSeconds / 60)).padStart(2, '0');
    const secs = String(webcamRecordingSeconds % 60).padStart(2, '0');
