...
try {
    switch ($action) {
        ...
        case 'assign_sku_by_code':
            $target_user_id = intval($_POST['target_user_id'] ?? 0);
            $sku_code = trim($_POST['sku_code'] ?? '');
            
            if (!$target_user_id || !$sku_code) {
                echo json_encode(['success' => false, 'message' => 'Faltan datos (usuario o código)']);
                break;
            }

            // Verificar si el SKU existe
            $stmt = $pdo->prepare("SELECT s.*, p.name as product_name, p.is_bulk, p.product_type FROM inventory_skus s JOIN inventory_products p ON s.product_id = p.id WHERE s.sku_code = ? AND s.is_deleted = 0");
            $stmt->execute([$sku_code]);
            $sku = $stmt->fetch();

            if ($sku) {
                if ($sku['status'] === 'instalado') {
                    echo json_encode(['success' => false, 'message' => 'El equipo ya está instalado']);
                    break;
                }
                if ($sku['assigned_to'] == $target_user_id) {
                    echo json_encode(['success' => true, 'message' => 'El equipo ya estaba en la mochila de este usuario']);
                    break;
                }
                
                // Asignar
                $upd = $pdo->prepare("UPDATE inventory_skus SET assigned_to = ?, status = 'asignado' WHERE id = ?");
                $upd->execute([$target_user_id, $sku['id']]);

                // Log
                $byId = intval($_SESSION['user_id'] ?? 0);
                $bn = $pdo->prepare("SELECT name FROM users WHERE id = ?"); $bn->execute([$byId]); $byName = $bn->fetchColumn() ?: '';
                
                $tu = $pdo->prepare("SELECT name FROM users WHERE id = ?"); $tu->execute([$target_user_id]); $tuName = $tu->fetchColumn() ?: '';

                $logStmt = $pdo->prepare("INSERT INTO inventory_assignment_log (sku_id, product_id, sku_code, product_name, assigned_to, assigned_to_name, assigned_by, assigned_by_name, quantity, is_epp, action, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1, 0, 'assigned', 'Asignación directa por escáner')");
                $logStmt->execute([$sku['id'], $sku['product_id'], $sku['sku_code'], $sku['product_name'], $target_user_id, $tuName, $byId, $byName]);

                echo json_encode(['success' => true, 'message' => "Equipo '".$sku['product_name']."' (".$sku['sku_code'].") asignado a la mochila"]);
                break;
            }

            // Si no es SKU directo, buscar si es un producto sin SKU (agrupado o a granel)
            $stmtProd = $pdo->prepare("SELECT * FROM inventory_products WHERE (sku = ? OR barcode = ?) AND is_deleted = 0");
            $stmtProd->execute([$sku_code, $sku_code]);
            $prod = $stmtProd->fetch();

            if ($prod) {
                echo json_encode(['success' => false, 'message' => "El producto '".$prod['name']."' no es un equipo individual serializado. Use asignación manual."]);
                break;
            }

            echo json_encode(['success' => false, 'message' => 'Código no encontrado en el inventario']);
            break;
